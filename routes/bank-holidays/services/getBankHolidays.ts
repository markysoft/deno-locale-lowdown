import { z } from 'zod'
import { getOneYearsTime } from '@/lib/utils.ts'
import { BankHoliday, BankHolidaySchema } from '../schemas/BankHoliday.ts'
...
export async function getBankHolidays(): Promise<BankHoliday[]> {
	...
	if (!holidaysResponse.ok) {
		throw new Error(
			`Failed to fetch bank holidays: ${holidaysResponse.statusText}`,
		)
	}
	const {
