...
export function getNextBusToMalton(currentTime: Date, busStop: string): string {
	return getNextBus(
		applyBritishSummerTime(currentTime),
		busStop,
		busTimes.toMalton,
	)
}

export function getNextBusFromMalton(
	currentTime: Date,
	busStop: string,
): string {
	return getNextBus(
...
export function getNextBus(
	...
	const nextStop = myStopTimes.find((time: BusTime) =>
		time.at > currentHourMins && time.daysOfWeek.includes(dayOfWeek)
	)
